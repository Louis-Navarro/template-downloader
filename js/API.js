			...
			if (matches && matches.length > 0) {
				...
				if (matches[4]) {
					...
					return new Promise(function(resolve, reject) {
						self
							._branchChecker(
								'https://api.github.com/repos/' + results.author + '/' + results.project + '/contents/',
								matches[5],
								matches[7]
							)
							.then(function(res) {
			...
			if (githubDownloadUrl.test(url)) return Promise.resolve(url);
				...
				return _githubUrlChecker.check(url).then(function(resolved) {
					return (
						...
						[ resolved.author, resolved.project, resolved.branch, resolved.path ].join('/')
					);
				});
		} else return Promise.reject();
	};
				...
				.then(function(xmlResponse) {
					...
					results.tree.forEach(function(item) {
						if (item.type == 'blob') {
							var p = {};
							promises.push(
								_getContentOfGitUrl(item.url, p).then(function(content) {
									var path = item.path;
						...
						_callAjax(
							...
							.then(function(xmlResponse) {
								...
								results.forEach(function(item) {
									if (item.type == 'dir') {
										...
										promises.push(
											_getTreeOfGitUrl(item.git_url).then(function(results) {
												// add currentPath
										...
										promises.push(Promise.resolve([ { url: item.git_url, path: item.name } ]));
										progressCallback.call(
											callbackScope,
											'processing',
							...
							.then(function(urls) {
								var fetches = urls.map(function(item) {
									return _getContentOfGitUrl(item.url).then(function(content) {
										var path = item.path;
										progressCallback.call(
											callbackScope,
											'processing',
